...
> **This is defense-in-depth, not a security boundary.** The tags and the notice are a *signal* to a cooperating model, not an enforceable delimiter. The wrapper carries attacker-controlled content verbatim, so a malicious value can embed a forged `</untrusted_agent_content>` closing tag or its own fake `SECURITY NOTICE:` line to try to break out of the wrapper or impersonate the server's framing. Treat the delimiter as advisory: escape or structurally encode untrusted values so an embedded tag cannot terminate the wrapper, and require every consumer to treat *all* returned content as data — never as instructions — regardless of how it is tagged. Tagging lowers the odds a well-behaved model is fooled; it does not stop a determined injection.
...
> **Provenance is classified by key name, which is a heuristic, not a guarantee.** Several keys on the system-generated list — `note`, `message`, `type`, `language`, `category`, `schedule` — are user- or agent-writable on many models. If a caller can write those fields, injection placed there is **not** tagged, because the classifier trusts the name rather than the value's origin. Name-based classification is a convenience for this demo surface, not a boundary. A hardened deployment should invert the default: **tag every string from a writable resource, and exempt only values it can prove are server-generated** — from the serializer/schema or an explicit provenance flag — rather than exempting by key name.
...
**It does not guarantee the delimiter holds.** Because the value is returned unchanged, a malicious value can contain a forged `</untrusted_agent_content>` closing tag or a counterfeit `SECURITY NOTICE:` line and attempt to break out of the wrapper. Unless untrusted values are escaped or structurally encoded before wrapping, the tags are advisory, not an enforceable boundary — every consumer must treat all returned content as data regardless of how it is tagged.
