...
| [RFC 7591](https://www.rfc-editor.org/rfc/rfc7591) | OAuth 2.0 Dynamic Client Registration Protocol | Implemented by `RegistrationView` (`/oauth/register/`). §2 defines the `grant_types` field enforced on `OAuthClient`. Disabled by default; enabled via `FRISIAN_MCP_OAUTH_REGISTRATION_OPEN`. |
| [RFC 7636](https://www.rfc-editor.org/rfc/rfc7636) | Proof Key for Code Exchange by OAuth Public Clients (PKCE) | PKCE S256 challenge/verifier, always applied on the authorization-code flow. `FRISIAN_MCP_OAUTH_PKCE_AUTO_REGISTER` controls whether an unknown public client may be auto-registered on first use of that flow — it does not toggle PKCE itself. |
...
| [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707) | Resource Indicators for OAuth 2.0 | Defines the OAuth `resource` parameter and resource identifiers used to scope a token to a specific protected resource. (The `/.well-known/oauth-protected-resource` URL construction itself is specified by RFC 9728, below.) |
