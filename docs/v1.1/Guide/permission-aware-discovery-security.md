...
> **`FRISIAN_MCP_OAUTH_SERVICE_USER` is a fallback — a per-client user takes precedence.** OAuth resolves `request.user` in priority order: the user set on the `OAuthClient` record (in the admin) first, then `FRISIAN_MCP_OAUTH_SERVICE_USER`, then a minimal service principal. So this global-`admin` setting makes *every OAuth client that has no per-client user* run as admin. Binding a specific, least-privileged user to each `OAuthClient` overrides the global service user for that client — that per-client binding, not the global setting, is where execution scoping belongs.
...
Here, anonymous callers see the `read_write`-tier tool surface in discovery (filtered by `AnonymousUser` permissions, which may be broad under some auth backends), but every tool they invoke executes as the `admin` account. This is **not** the same as exposing the admin tool surface — admin-tier tools stay hidden behind the `read_write` ceiling — but it is still severe: each read/write tool the caller *can* see runs with admin's underlying data permissions, so anyone with network access can read and modify whatever the admin account can through those visible tools. Keep the discovery tier and the execution identity aligned; do not treat the tier ceiling as a bound on a broad execution account.
