...
If `FRISIAN_MCP_PERMISSION_CLASSES = ["rest_framework.permissions.IsAuthenticated"]` is set, unauthenticated requests receive 401 by design. Note that `IsAuthenticated` only establishes *who* the caller is — it is an authentication gate, not an authorization boundary. It does not by itself restrict *which objects* an authenticated caller may read or write; on Nautobot that object-level authorization still comes from the host's per-user queryset scoping (`restrict()`) and object permissions. Do not treat authentication-only access as a substitute for those. It is the correct gateway posture for a production deployment, paired with the host's object permissions behind it.
