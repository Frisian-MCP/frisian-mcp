...
**`invocation.py` — body is forwarded as-is.** `SyncInvocation.invoke()` calls `_extract_list_body(body_args)`. That function checks whether the single key is one of the wrapper keys named by `_LIST_BODY_KEYS` in `invocation.py`, and the value is a **list** for bulk-create. The members are deliberately not restated here: the dispatcher's `help` now publishes that set to callers as fact, so a copy in prose is a copy that can drift and make the documentation authoritative and wrong. Read the constant. `{"email": "...", "first_name": "..."}` is a dict, not a list, so `_extract_list_body` returns `None`. FK normalization runs on `body_args` but `data` is not in the tool's schema properties, so it is passed through unchanged. DRF receives `{"data": {"email": "...", "first_name": "..."}}`, its serializer encounters an undeclared `data` field, silently ignores it, and creates the record with all fields absent or at their empty defaults.
