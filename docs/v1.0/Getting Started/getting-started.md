...
For `uv` inside Docker, install `uv` in the build stage and use `uv pip install --system` so the runtime keeps using system Python (no venv path surgery). This installs the latest published `frisian-mcp` and is **not** lockfile-pinned:
...
For a reproducible, lockfile-pinned build instead, copy your project's `pyproject.toml` and `uv.lock` and run `uv sync --frozen`, which installs the exact locked versions into a project environment.
...
> The trailing slash is significant. Mount at `mcp/` and connect to `…/mcp/`. Some MCP clients do not follow the Django `APPEND_SLASH` redirect from `/mcp` to `/mcp/` during initialization, so give clients the exact path with the slash.
...
When `FRISIAN_MCP_AUTODISCOVER = True`, frisian-mcp walks your DRF router and registers each ViewSet as an MCP tool. The scan is deferred to the first request (so it captures late-loading plugin ViewSets), and it introspects your routers, ViewSets, and serializers directly — there is no OpenAPI schema step required.
...
```python
# Keys are stored as HMAC-SHA256 digests, not raw values. Generate each with
#   python manage.py mcp_hash_api_key <raw-key>
# and paste the resulting 64-char digest here:
FRISIAN_MCP_API_KEYS = {
    "<64-hex digest of your agent key>":    'read_write',
    "<64-hex digest of your readonly key>": 'read',
}
```

An agent sends the **raw** key as a Bearer token; the package HMACs it and matches the stored digest. Because only digests live in settings, a leaked settings file exposes no usable credentials.
...
Implements OAuth 2.0 with RFC 8414 well-known metadata and RFC 7591 dynamic client registration, supporting both grant types: **Authorization Code + PKCE** for browser and native clients (Claude.ai, and coding agents configured for OAuth), and **`client_credentials`** for service-to-service callers. Once configured, a client discovers the metadata endpoint and uses whichever flow fits — see the [connection guide](../Guide/connect-agent.md) for the per-client walkthrough.
...
- **Changelog** — full version history from v0.1.0 to the current release
