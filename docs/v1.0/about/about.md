...
No changes to urls.py. No modifications to existing models, serializers, or permissions. No middleware injected into the host's request pipeline.  The entire integration lives inside the installed package. How it mounts depends on the host: on a standard Django project it self-registers via `AppConfig.ready()`; on plugin hosts it rides the host's plugin system instead (a thin plugin wrapper on NetBox, `PluginURLs` on Open edX — see the per-host install guides).
