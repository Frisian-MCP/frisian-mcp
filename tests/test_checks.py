"""
SEC-4 — Django system check for empty FRISIAN_MCP_PERMISSION_CLASSES in production.

The check (``frisian_mcp.W001``) runs when ``DEBUG=False`` and refuses to be
silent about an unauthenticated MCP gateway.  Operators who want an open
gateway must set ``FRISIAN_MCP_ALLOW_UNAUTHENTICATED=True`` to acknowledge
