...
- **`FRISIAN_MCP_OAUTH_AUTO_APPROVE` is reframed as "remember consent."** The setting no longer means "skip the consent form" — it now means "remember consent for repeat grants of the same `(user, client_id, redirect_uri, scope)` tuple." The first authorize for any new tuple renders the consent form regardless of `AUTO_APPROVE`; subsequent authorizes for the same tuple fast-path on the stored consent. The DEBUG-derived default (`bool(DEBUG)`) is removed; the default is now `False` unconditionally. A new `OAuthAuthorizeConsent` model records each granted consent and is admin-browsable with a `revoke_selected_consents` bulk action. Operators with machine-to-machine flows that cannot render a consent form must pre-populate `OAuthAuthorizeConsent` records via the admin to preserve silent code issuance.
