"""Contrib middleware for the frisian-mcp tool call middleware system."""
...
def _load_backend(dotted_path: str) -> AbstractRateLimitBackend:
    ...
    try:
        ...
        raise ImproperlyConfigured(
            f"FRISIAN_MCP_RATE_LIMIT 'backend' must be a dotted import path, "
            f"got {dotted_path!r}."
        ) from exc
...
class RateLimitMiddleware:
    ...
    Plugs into ``FRISIAN_MCP_TOOL_MIDDLEWARE`` and reads configuration from
    ``FRISIAN_MCP_RATE_LIMIT``::

        FRISIAN_MCP_TOOL_MIDDLEWARE = [
            "frisian_mcp.contrib.middleware.RateLimitMiddleware",
        ]
    ...
    Raises:
        :exc:`~django.core.exceptions.ImproperlyConfigured`: When
            ``FRISIAN_MCP_RATE_LIMIT`` is present but ``rate`` has an invalid
    ...
    def __init__(self) -> None:
        ...
        config: dict[str, Any] | None = getattr(settings, "FRISIAN_MCP_RATE_LIMIT", None)
        ...
        self._proxy_count: int = getattr(settings, "FRISIAN_MCP_TRUSTED_PROXY_COUNT", 0)
        ...
        if config is not None:
            ...
            if not match:
                raise ImproperlyConfigured(
                    f"FRISIAN_MCP_RATE_LIMIT: invalid rate {rate_str!r}. "
