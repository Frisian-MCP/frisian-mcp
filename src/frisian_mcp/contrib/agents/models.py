...
Requirements
~~~~~~~~~~~~
This module requires both ``frisian_mcp.contrib.tokens`` and
``frisian_mcp.contrib.oauth`` to be present in ``INSTALLED_APPS`` so that the
foreign key migrations can resolve.  Install at least one credential app before
running ``migrate``::

    INSTALLED_APPS = [
        ...
        "frisian_mcp.contrib.tokens",   # and/or contrib.oauth
...
Usage::
    ...
    from frisian_mcp.contrib.agents.models import AgentConnection

    token = FrisianMcpToken.objects.create(name="claude-agent")
...
class AgentConnection(models.Model):
    ...
    token = models.ForeignKey(
        "frisian_mcp_tokens.FrisianMcpToken",
        null=True,
        blank=True,
