"""
frisian_mcp.backends — pluggable discovery and invocation backends.

Public API::

    from frisian_mcp.backends import (
...
from frisian_mcp.backends.base import (
    BaseDiscoveryBackend,
    BaseInvocationBackend,
    ToolDefinition,
    ToolResult,
...
def get_discovery_backend(
    ...
    Reads ``settings.FRISIAN_MCP_DISCOVERY_BACKEND``.  Falls back to *default*,
    ...
    """
    class_path: str | None = getattr(settings, "FRISIAN_MCP_DISCOVERY_BACKEND", None)
...
def get_discovery_backends() -> list[BaseDiscoveryBackend]:
    ...
    1. ``FRISIAN_MCP_DISCOVERY_BACKENDS`` — a list of dotted-path strings, each
    ...
    2. ``FRISIAN_MCP_DISCOVERY_BACKEND`` (singular, legacy) — wrapped in a
    ...
    """
    plural: list[str] | None = getattr(settings, "FRISIAN_MCP_DISCOVERY_BACKENDS", None)
    if plural is not None:
        return [_load_class(path)() for path in plural]

    singular: str | None = getattr(settings, "FRISIAN_MCP_DISCOVERY_BACKEND", None)
...
def get_invocation_backend(
    ...
    Reads ``settings.FRISIAN_MCP_INVOCATION_BACKEND``.  Falls back to *default*,
    ...
    """
    class_path: str | None = getattr(settings, "FRISIAN_MCP_INVOCATION_BACKEND", None)
    if class_path is not None:
        cls = _load_class(class_path)
        return cls()  # type: ignore[no-any-return]
    if default is not None:
