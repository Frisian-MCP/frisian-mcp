"""
Middleware utilities for frisian-mcp.
...
1. **Tool-call middleware chain** — a per-tool callable pipeline configured via
   :setting:`FRISIAN_MCP_TOOL_MIDDLEWARE`.  See :func:`load_middleware` and
...
2. **Django HTTP request middleware** — :class:`McpTrailingSlashMiddleware`
   ...
   :class:`~frisian_mcp.apps.FrisianMcpConfig.ready`.
"""
...
#: ``settings.FRISIAN_MCP_URL_PREFIX`` (e.g. ``"/api/mcp"``).
DEFAULT_MCP_URL_PREFIX: str = "/mcp"


def load_middleware() -> list[Any]:
    """
    Load and instantiate middleware classes from ``FRISIAN_MCP_TOOL_MIDDLEWARE``.
    ...
    paths: list[str] = getattr(settings, "FRISIAN_MCP_TOOL_MIDDLEWARE", [])
    instances: list[Any] = []
    for path in paths:
        module_path, _, class_name = path.rpartition(".")
        if not module_path:
            raise ImproperlyConfigured(
                f"FRISIAN_MCP_TOOL_MIDDLEWARE: {path!r} is not a valid dotted Python path"
            )
        try:
            module = importlib.import_module(module_path)
        except ImportError as exc:
            raise ImproperlyConfigured(
                f"FRISIAN_MCP_TOOL_MIDDLEWARE: could not import {path!r}: {exc}"
            ) from exc
        cls = getattr(module, class_name, None)
        if cls is None:
            raise ImproperlyConfigured(
                f"FRISIAN_MCP_TOOL_MIDDLEWARE: {module_path!r} has no attribute {class_name!r}"
            )
...
def _get_mcp_url_prefix() -> str:
    ...
    Reads :setting:`FRISIAN_MCP_URL_PREFIX` (default :data:`DEFAULT_MCP_URL_PREFIX`)
    ...
    """
    prefix: str = getattr(settings, "FRISIAN_MCP_URL_PREFIX", DEFAULT_MCP_URL_PREFIX)
...
class McpTrailingSlashMiddleware:
    ...
    similarly for any configured prefix).  The frisian-mcp URLconf accepts both
    ...
    Configuration:
        * ``FRISIAN_MCP_URL_PREFIX`` — absolute URL prefix at which the gateway
          is mounted.  Defaults to :data:`DEFAULT_MCP_URL_PREFIX`.

    Auto-installation:
        :meth:`frisian_mcp.apps.FrisianMcpConfig.ready` inserts this middleware
